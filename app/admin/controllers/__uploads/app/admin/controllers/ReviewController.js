...
const Reviewfilt = new Database("Dreview"
    // , [
    //     "id",
    //     "reviewkey",
    //     "agentid",
...
module.exports = {
        ...
        return Review.insert(ctx, {
            ...
            .then(res => {
                return ctx.call('review.getAll').then(res => {
                    return this.requestSuccess("Review Successfly Created", res.data);
                });
            })
            .catch((err) => {
    ...
    statuschange: function(ctx) {
        ...
        findreview['status'] = ctx.params.status ? ctx.params.status : {
            [Op.ne]: DELETE
        };
    ...
    getAll: async function(ctx) {
        return await sequelize12.query('exec sp_getratingsandreview', { type: Sequ.QueryTypes.SELECT })
            .then(res => {
                if (res)
                    return this.requestSuccess("Review and rating List", res);
                else
                    return this.requestError(CodeTypes.NOTHING_FOUND);
            })
            .catch(err => {
    ...
    admin_list: async function(ctx) {
        let playersList = await sequelize12.query('EXEC SP_agentReviewDetail :agentid,:rating,:review,:name', { replacements: { agentid: ctx.params.agentid, rating: ctx.params.rating, review: ctx.params.review, name: ctx.params.username, }, type: Sequ.QueryTypes.SELECT });
            ...
            .then((res) => {
                ...
                async function get_agentdetails(ctx, arr) {
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        let language_val_filter = await Agentlangfilt.find(ctx, { query: { agentid: arr[i].agentid, langshortname: ctx.options.parentCtx.params.req.headers.language } })
                            .then((lan_res) => {
                                arr[i]["agentname"] = lan_res.data[0].agentname;
                                return arr[i];
                            });
                        // to get agent images
                        let agent_image = await Agentfilt.find(ctx, { query: { id: arr[i].agentid } })
                            .then((images) => {
                                let image_arr = " ";
    ...
    review_approval: function(ctx) {
        ...
        return Review.findOne(ctx, {
            ...
            .then((res) => {
                Review.updateBy(ctx, res.data.id, {
    ...
    update: function(ctx) {
        ...
        findcountry['countryname'] = ctx.params.language[0].countryname ? ctx.params.language[0].countryname : {
            [Op.ne]: DELETE
        };
        findcountry['status'] = ctx.params.status ? ctx.params.status : {
            [Op.ne]: DELETE
        };
