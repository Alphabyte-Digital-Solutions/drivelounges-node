...
module.exports = {
    ...
    user_create: async function(ctx) {
        ...
        return await sequelize12.query('EXEC sp_checkUserAlreadyExist @email=:email,@contact=:contact', 
            { replacements: {email: ctx.params.email,contact : ctx.params.contactnumber}, 
            ...
            .then(res => {
                ...
                if(res.length == 0){
                    ...
                    .then(response => {
                        ...
                        readHTMLFile(mail_template + "/UsertemplateOTP.html", function(err, html) {
                        ...
                        return this.requestSuccess("User Created successfully", ctx.params.email);
            ...
            .then(res => {
                if(res.length == 0){
                    ...
                    .then(response => {
                        ...
                        return User.find(ctx, {query: {usertypeid: 1, status: 1}}).then(res => {
                        ...
                        return User.insert(ctx, {
                            ...
                            readHTMLFile(mail_template + "/Usertemplate.html", function(err, html) {
                                ...
                                ctx.call("mail.send", {
                                    ...
                                    // return "Email send Successfully";      
                                    return await sequelize12.query('EXEC sp_getCustomerDetails @status=:status', { replacements: { status: 1 }, type: Sequ.QueryTypes.SELECT })
                                    .then(res => {
                                        console.log(res)
    ...
    admin_user_update: async function(ctx) {
        ...
        .then(async response => {
           return await sequelize12.query('EXEC sp_getCustomerDetails @status=:status', { replacements: { status: 1 }, type: Sequ.QueryTypes.SELECT }).then(res => {
               return this.requestSuccess("Email sent successfully", res);
           }).catch((err) => {
               console.log(err)
               if (err.name === 'Nothing Found')
                 ...
                 if (ctx.params.otp.localeCompare(res.data.otp) == 0) {
                    return User.updateBy(ctx, res.data.id, {
                        ...
                        .then(user => {
                            ...
                            return this.requestSuccess("User logged successfully", ctx.params.email);
                        })
                } else {
                    return this.requestError(CodeTypes.NOTHING_FOUND);
                }
        ...
        return dirDetails(dirLink)
            ...
            .then((r) => {
                ...
                const get_AnnnotationDetails = async(x, link) => {
                    return path.extname(link + '/' + x) == '.js' ? annotations.get(link + '/' + x).then((res) => {
    ...
    changeInfo: function(ctx) {
        return User.find(ctx, { query: { id: ctx.params.id, status: 1 } }).then(res => {
                if (res.data) {
                    return User.updateBy(ctx, ctx.params.id, {
                        firstname: ctx.params.firstname,
                        lastname: ctx.params.lastname,
    ...
    user_changePassword: function(ctx) {
        ...
        // });
        return this.verifyIfLogged(ctx)
            .then(() => ctx.call("auth.verifyuser_change_Password", { id: ctx.params.id, password: ctx.params.oldpassword, newpassword: ctx.params.newpassword }))
    ...
    getuserbyid: function(ctx) {
        return User.find(ctx, { query: { id: ctx.params.id, usertypeid: ctx.params.usertypeid } }).then(res => {
    ...
    update: function(ctx) {
        return Agent.find(ctx, {
            ...
            .then((agent) => {
                if (agent) {
                    return Agent.updateBy(ctx, ctx.params.id, {
                        firstname: ctx.params.firstname,
                        lastname: ctx.params.lastname,
                        username: ctx.params.username,
                        email: ctx.params.email,
                ...
                .then(response => {
                    AgentLang.updateBy(ctx, ctx.params.id, {
                        languageid: ctx.params.languageid,
                        langshortname: ctx.params.languageid == 1 ? "en" : "ar",
                        agentname: ctx.params.agentname,
                        agentdescription: ctx.params.agentdescription,
                    ...
                    return Agent.find(ctx, {query: {status : 1}})
                    .then(res => {
                        return this.requestSuccess("List of Agencies", res.data);
                    })
                })
    ...
    agent_changePassword: function(ctx) {
        ...
        // });
        return this.verifyIfLogged(ctx)
            .then(() => ctx.call("auth.verifyagent_change_Password", { id: ctx.params.id, password: ctx.params.oldpassword, newpassword: ctx.params.newpassword }))
            .then((res) => {
                if (ctx.params.newpassword.localeCompare(ctx.params.confirmpassword) == 0) {
    ...
    agent_getApproved: function(ctx) {
        return Agent.find(ctx, { query: {status: 1, agentstatus: 1, isverified: 1} })
            .then(res => {
                return this.requestSuccess("List of Agencies", res.data);
            })
            .catch((err) => {
        ...
        return Agent.findOne(ctx, {
            ...
            .then(res => {
                return this.generateHash(rande[0]).then(res_pass =>{
                    return Agent.updateBy(ctx, res.data.id, {
                        ...
                        ctx.meta.log = ctx.params.agentstatus == 1 ? 'Agency approval' : "Agency rejected";
                        activity.setLog(ctx);
                        let readHTMLFile = function(path, callback) {
                            fs.readFile(path, { encoding: 'utf-8' }, function(err, html) {
                                if (err) {
                        ...
                        readHTMLFile(mail_template + "/Requesttemplate.html", function(err, html) {    
                            ...
                            ctx.call("mail.send", {
                                ...
                                subject: ctx.params.agentstatus == 1 ? 'Agency approval' : "Agency rejected",
                                html: htmlToSend
                            }).then((res) => {
                                return "Email send Successfully";
                            })
    ...
    agent_getbyid: async function(ctx) {
        return await sequelize12.query('exec sp_getagentbyid @id=:id', {
                replacements: { id: ctx.params.id },
                type: Sequ.QueryTypes.SELECT
            })
            .then(res => {
                if (res)
                    return this.requestSuccess("Agency Information", res);
                else
                    return this.requestError(CodeTypes.NOTHING_FOUND);
            })
            .catch(err => {
    ...
    getuserreview: async function(ctx) {
        return await sequelize12.query('exec sp_getreivewbyid @Id=:id,@UserType=:usertypeid', {
