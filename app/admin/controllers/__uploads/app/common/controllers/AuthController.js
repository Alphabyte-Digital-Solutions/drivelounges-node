    ...
    agent_resetPassword: function(ctx) {
        ...
        return agent.find(ctx, {
            ...
            .then((response) => {
                if (response.data) {
                    //generateHash gives encrypted password
                    //console.log('agent pass: ',rande[0]);
                    return this.generateHash(rande[0])
                        .then((res) => {
                            return agent.updateBy(ctx, 1, { password: res.data, otp: rande[0] }, {
                                query: {
                                    id: response.data[0].id
                                }
                            })
                            ...
                            .then((result) => {
                                ctx.meta.username = ctx.params.email;
                                ctx.meta.log = 'agent Password has been reseted';
                                activity.setLog(ctx);
                                let readHTMLFile = function(path, callback) {
                                    fs.readFile(path, { encoding: 'utf-8' }, function(err, html) {
                                        if (err) {
