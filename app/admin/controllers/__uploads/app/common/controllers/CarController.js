...
module.exports = {
    ...
    get_carmanagement: async function(ctx) {
        return await sequelize12.query('exec sp_getcarmanagement @carstatus=:carstatus', {
                replacements: { carstatus: ctx.params.carstatus },
                type: Sequ.QueryTypes.SELECT
            })
            .then(res => {
                if (res)
    ...
    get_carfullinfolist: async function(ctx) {
        return await sequelize12.query('EXEC sp_getcarfullinfo @Lang=:id', { replacements: { id: ctx.params.Lang }, type: Sequ.QueryTypes.SELECT })
            .then(res => {
                return this.requestSuccess("Car Display Information List", res);
            })
            .catch(err => {
